...
spec = describe "Functions" $
  it "contains only unique bindings after 'join'" $ do
    let first = ("B1", MvBindings [BiVoid AtRho, BiDelta BtEmpty, BiTau (AtLabel "x") ExRoot, BiVoid (AtAlpha 0)])
        second = ("B2", MvBindings [BiTau AtRho ExXi, BiLambda "Func", BiDelta (BtOne "00"), BiVoid (AtAlpha 1)])
        third = ("B3", MvBindings [BiLambda "Some", BiTau (AtLabel "y") ExXi, BiTau (AtLabel "x") ExXi, BiVoid (AtAlpha 0)])
