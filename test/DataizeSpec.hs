...
spec = do
  ...
  describe "stops a dataization that never reaches bytes" $ do
    it "fails on the step limit instead of morphing forever" $
      looping $ \endless -> do
        ...
        dataize expr emptyState (ReduceContext ExRoot ExRoot Nothing noNormals 25 25 (Steps 40 0) 1 False True False False Nothing Dataization [] Map.empty endless buildTerm reduction evaluation fired dontSaveStep dontSaveEval)
    ...
    it "parks the step limit as a residual with --partial" $
      looping $ \endless -> do
        expr <- parseExpressionThrows "⟦ @ ↦ ⟦ λ ⤍ L_loop ⟧ ⟧"
        (outcome, _, _) <- dataize expr emptyState (ReduceContext ExRoot ExRoot Nothing noNormals 25 25 (Steps 40 0) 1 False True True False Nothing Dataization [] Map.empty endless buildTerm reduction evaluation fired dontSaveStep dontSaveEval)
        case outcome of
          Residual _ -> pure ()
  ...
  describe "ReduceContext's --max-depth/--max-cycles reach into the normalization it splices in" $ do
    ...
    forM_
      [
        ( "--max-cycles"
        , ReduceContext ExRoot ExRoot Nothing noNormals 25 0 (Steps 250 0) 1 True True False False Nothing Dataization [] Map.empty emptyLambdas buildTerm reduction evaluation fired dontSaveStep dontSaveEval
        , "--max-cycles=0"
        )
      ,
        ( "--max-depth"
        , ReduceContext ExRoot ExRoot Nothing noNormals 0 25 (Steps 250 0) 1 True True False False Nothing Dataization [] Map.empty emptyLambdas buildTerm reduction evaluation fired dontSaveStep dontSaveEval
        , "--max-depth=0"
        )
      ]
      ( \(flag, ctx, message) ->
    ...
    forM_
      [ ("--max-cycles", ReduceContext ExRoot ExRoot Nothing noNormals 25 0 (Steps 250 0) 1 False True False False Nothing Dataization [] Map.empty emptyLambdas buildTerm reduction evaluation fired dontSaveStep dontSaveEval)
      , ("--max-depth", ReduceContext ExRoot ExRoot Nothing noNormals 0 25 (Steps 250 0) 1 False True False False Nothing Dataization [] Map.empty emptyLambdas buildTerm reduction evaluation fired dontSaveStep dontSaveEval)
      ]
      ( \(flag, ctx) ->
