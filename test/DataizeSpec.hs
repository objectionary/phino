...
import Fixtures (defaultReduceContext, fixtureFunctions, loopingFunctions, primitives, recorded, withFunctions)
...
-- so and run against the fixture λ functions where 'functions' does, entered at
...
testDataize known pth = do
  ...
  let ctx = (defaultReduceContext loc){_functions = if functions == Just True then known else emptyLambdas}
...
-- Dataize under '--partial', handing back the protocol of '--evaluations'
-- alongside the answer, one record per line
partially :: Lambdas -> String -> IO ((Outcome, [Rewritten]), [String])
partially known src = do
  expr <- parseExpressionThrows (primitives src)
  recorded $ \record -> do
    let ctx = (withFunctions known (defaultReduceContext ExRoot)){_partial = True, _saveEval = record}
    (outcome, chain, _) <- dataize expr emptyState ctx
    pure (outcome, chain)

-- The one λ function that answers with a firing of itself, read the way
-- '--functions' reads it, so that a run fires it until the step budget is gone
looping :: (Lambdas -> IO a) -> IO a
looping action = loopingFunctions (readLambdas >=> action)
...
spec = do
  ...
  -- with '--functions' decides, and each entry of it answers the firing with a
  -- term of the calculus (see 'Lambdas'). What a pack with 'functions' on
  -- asserts is that such an answer lands in the derivation exactly where a
  -- built-in λ function's answer used to: 𝔼 normalizes it and 𝔻 carries on. The λ
  -- functions themselves are the fixture ones (see 'Fixtures'), and 'number.eq'
  -- is composed out of 'L_bytes_eq' the way 'eq.eo' composes it, so the
  -- EO-level composition is exercised too.
  describe "dataize" $ do
  ...
  describe "stops a dataization that never reaches bytes" $ do
    it "fails on the step limit instead of morphing forever" $
      looping $ \endless -> do
        ...
        dataize expr emptyState (ReduceContext ExRoot 25 25 (Steps 40 0) False True False False endless buildTerm reduction dontSaveStep dontSaveEval)
    ...
    it "parks the step limit as a residual with --partial" $
      looping $ \endless -> do
        expr <- parseExpressionThrows "⟦ @ ↦ ⟦ λ ⤍ L_loop ⟧ ⟧"
        (outcome, _, _) <- dataize expr emptyState (ReduceContext ExRoot 25 25 (Steps 40 0) False True True False endless buildTerm reduction dontSaveStep dontSaveEval)
        case outcome of
          Residual _ -> pure ()
          Dataized bts -> expectationFailure ("expected a residual, dataized to " ++ show bts)

  -- A λ function no entry of the '--functions' file answers — a name the file
  ...
  -- already evaluated, while the protocol of '--evaluations' names each parked
  -- site. An operand that cannot be reduced is the entry's own business, so
  -- what parks here is the unanswered λ function alone.
  describe "partially evaluates around a λ function that cannot fire (--partial)" $ do
    ...
    it "fails on it without the flag, naming the λ function" $ do
      expr <- parseExpressionThrows (primitives "2.plus(3).nope")
      dataize expr emptyState (withFunctions known (defaultReduceContext ExRoot))
        `shouldThrow` (\e -> "No entry of --functions answers the λ function 'L_number_nope'" `isInfixOf` show (e :: SomeException))
    it "leaves the application of the unanswered λ function in place" $ do
      ((outcome, _), _) <- partially known "2.plus(3).nope"
      case outcome of
    ...
    it "keeps what was evaluated before the stuck site in the residue" $ do
      ...
      case outcome of
        Residual (ExFormation bds) -> do
          ...
          -- 2 + 3 = 5.0, whose IEEE 754 bytes are 40-14-00-00-00-00-00-00
          show rho `shouldContain` show (BtMany ["40", "14", "00", "00", "00", "00", "00", "00"])
          -- the plus application is gone: ρ is the number it produced, its 'as-bytes' bound
    ...
    it "leaves an unanswered λ function dataized directly as the whole residue" $ do
      ((outcome, chain), reports) <- partially known "[[ L> Sym_arg_0 ]]"
      outcome `shouldBe` Residual placeholder
      reports `shouldBe` ["{\"λ\":\"Sym_arg_0\",\"stuck\":true}"]
      map fst chain `shouldEndWith` [placeholder]
    it "still reaches bytes when nothing is stuck" $ do
      ((outcome, _), reports) <- partially known "2.plus(3)"
      outcome `shouldBe` Dataized (BtMany ["40", "14", "00", "00", "00", "00", "00", "00"])
      length reports `shouldBe` 1
    it "stops on the terminator ⊥ as before, since a wrong operand is not a stuck λ function" $ do
      expr <- parseExpressionThrows (primitives "5.plus( Φ.bytes( φ ↦ ⟦ Δ ⤍ -- ⟧ ) )")
      dataize expr emptyState ((withFunctions known (defaultReduceContext ExRoot)){_partial = True})
        `shouldThrow` (\e -> "terminator" `isInfixOf` show (e :: SomeException))
  ...
  describe "ReduceContext's --max-depth/--max-cycles reach into the normalization it splices in" $ do
    ...
    forM_
      [
        ( "--max-cycles"
        , ReduceContext ExRoot 25 0 (Steps 250 0) True True False False emptyLambdas buildTerm reduction dontSaveStep dontSaveEval
        , "--max-cycles=0"
        )
      ,
        ( "--max-depth"
        , ReduceContext ExRoot 0 25 (Steps 250 0) True True False False emptyLambdas buildTerm reduction dontSaveStep dontSaveEval
        , "--max-depth=0"
        )
      ]
      ( \(flag, ctx, message) ->
    ...
    forM_
      [ ("--max-cycles", ReduceContext ExRoot 25 0 (Steps 250 0) False True False False emptyLambdas buildTerm reduction dontSaveStep dontSaveEval)
      , ("--max-depth", ReduceContext ExRoot 0 25 (Steps 250 0) False True False False emptyLambdas buildTerm reduction dontSaveStep dontSaveEval)
      ]
      ( \(flag, ctx) ->
  ...
  describe "labels every step with a defined rule or operation" $ do
    ...
    it "uses no step label without a defining rule or operation" $ do
      ...
      (_, chain, _) <- dataize expr emptyState (withFunctions known (defaultReduceContext loc))
