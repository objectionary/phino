      ...
      withNode $ do
        ...
        case outcome of
          Residual (ExFormation bds) -> do
            ...
            -- the times application is gone: ρ is the number it produced, its 'φ' bound
            [() | ExFormation inner <- rho, BiTau AtPhi _ <- inner] `shouldBe` [()]
          other -> expectationFailure ("expected a residual formation, got " ++ show other)
    ...
    it "dataizes 5.plus(6) through the expected rules" $
      withNode $ do
        labels <-
          labelsOf
            "Q"
            "[[ bytes ↦ ⟦ φ ↦ ∅ ⟧, number(φ) -> [[ as-bytes -> $.φ, plus(x) -> [[ L> L_number_plus ]] ]], @ -> 5.plus(6) ]]"
        labels
