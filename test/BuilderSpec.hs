...
spec = do
  ...
  describe "objectOf" $ do
    it "builds a top-level object as the world declares it" $
      objectOf
        (ExFormation [BiTau (AtLabel "qv") (ExFormation [BiVoid AtPhi, BiTau (AtLabel "wz") (ExFormation [])])])
        (ExDispatch ExRoot (AtLabel "qv"))
        `shouldBe` Just (ExFormation [BiVoid AtPhi, BiTau (AtLabel "wz") (ExFormation [])])
    it "fills the void an application of the object hands a closed term" $
      objectOf
        (ExFormation [BiTau (AtLabel "qv") (ExFormation [BiVoid AtPhi, BiTau (AtLabel "wz") (ExFormation [])])])
        (ExApplication (ExDispatch ExRoot (AtLabel "qv")) (ArTau AtPhi (ExFormation [BiDelta (BtOne "1F")])))
        `shouldBe` Just (ExFormation [BiTau AtPhi (ExFormation [BiDelta (BtOne "1F")]), BiTau (AtLabel "wz") (ExFormation [])])
    it "decorates a method with the path of the object it was dispatched off" $
      objectOf
        (ExFormation [BiTau (AtLabel "qv") (ExFormation [BiVoid AtPhi, BiTau (AtLabel "wz") (ExFormation [BiVoid AtRho, BiLambda (Function "L_wz")])])])
        (ExDispatch (ExApplication (ExDispatch ExRoot (AtLabel "qv")) (ArTau AtPhi ExRoot)) (AtLabel "wz"))
        `shouldBe` Just (ExFormation [BiTau AtRho (ExApplication (ExDispatch ExRoot (AtLabel "qv")) (ArTau AtPhi ExRoot)), BiLambda (Function "L_wz")])
    it "fills the ρ of a top-level object with Φ" $
      objectOf
        (ExFormation [BiTau (AtLabel "jx") (ExFormation [BiVoid AtRho, BiDelta (BtOne "7A")])])
        (ExDispatch ExRoot (AtLabel "jx"))
        `shouldBe` Just (ExFormation [BiTau AtRho ExRoot, BiDelta (BtOne "7A")])
    it "does not build an attribute bound to a term other than a formation" $
      objectOf
        (ExFormation [BiTau (AtLabel "qv") (ExDispatch ExRoot (AtLabel "zz"))])
        (ExDispatch ExRoot (AtLabel "qv"))
        `shouldBe` Nothing
    it "does not fill a void with a term that reaches its own ξ" $
      objectOf
        (ExFormation [BiTau (AtLabel "qv") (ExFormation [BiVoid AtPhi])])
        (ExApplication (ExDispatch ExRoot (AtLabel "qv")) (ArTau AtPhi ExXi))
        `shouldBe` Nothing
    it "keeps the ρ a dispatch has filled when an application hands another" $
      objectOf
        (ExFormation [BiTau (AtLabel "qv") (ExFormation [BiVoid AtRho, BiVoid (AtLabel "yu")])])
        (ExApplication (ExDispatch ExRoot (AtLabel "qv")) (ArTau AtRho (ExFormation [BiDelta (BtOne "0C")])))
        `shouldBe` Just (ExFormation [BiTau AtRho ExRoot, BiVoid (AtLabel "yu")])
    it "leaves an object without ρ as it was when an application hands one" $
      objectOf
        (ExFormation [BiTau (AtLabel "qv") (ExFormation [BiVoid (AtLabel "yu")])])
        (ExApplication (ExDispatch ExRoot (AtLabel "qv")) (ArTau AtRho (ExFormation [])))
        `shouldBe` Just (ExFormation [BiVoid (AtLabel "yu")])
    it "does not build an object holding both λ and Δ" $
      objectOf
        (ExFormation [BiTau (AtLabel "qv") (ExFormation [BiDelta (BtOne "00"), BiLambda (Function "L_qv"), BiTau (AtLabel "k") (ExFormation [])])])
        (ExDispatch (ExDispatch ExRoot (AtLabel "qv")) (AtLabel "k"))
        `shouldBe` Nothing
    it "does not name Φ itself as an object" $
      objectOf (ExFormation [BiTau (AtLabel "qv") (ExFormation [])]) ExRoot `shouldBe` Nothing
