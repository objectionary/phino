...
spec = do
  ...
  describe "parses correctly" $
    forM_
      [ ("in(!a1, !B1)", Y.In (AtMeta (Just "a1")) (BiMeta (Just "B1")))
      , ("not(in(!a1,!B1))", Y.Not (Y.In (AtMeta (Just "a1")) (BiMeta (Just "B1"))))
      , ("alpha(y)", Y.Alpha (AtLabel "y"))
      , ("eq(1,-2)", Y.Eq (Y.CmpNum (Y.Literal 1)) (Y.CmpNum (Y.Literal (-2))))
      , ("eq(index(z),length(!B1))", Y.Eq (Y.CmpNum (Y.Index (AtLabel "z"))) (Y.CmpNum (Y.Length (BiMeta (Just "B1")))))
      , ("eq(!a1, !e2)", Y.Eq (Y.CmpAttr (AtMeta (Just "a1"))) (Y.CmpExpr (ExMeta (Just "e2"))))
      , ("or(xi(!e1), nf(Q.x))", Y.Or [Y.Xi (ExMeta (Just "e1")), Y.NF (ExDispatch ExGlobal (AtLabel "x"))])
      , ("and(matches(\"hi\", !e1),part-of(!e1, !B1))", Y.And [Y.Matches "hi" (ExMeta (Just "e1")), Y.PartOf (ExMeta (Just "e1")) (BiMeta (Just "B1"))])
      ]
