...
import Fixtures (defaultReduceContext, fixtureFunctions, primitives, recorded, withFunctions, withFunctionsOf)
...
-- against the fixture λ functions where 'functions' does, entered at 'location'
...
testMorph known deep pth = do
  ...
  let ctx =
        (defaultReduceContext loc)
          ...
          , _functions = if functions == Just True then known else emptyLambdas
          }
...
-- One case of a λ function answered by the '--functions' file, as a pack of
-- 'test-resources/morph-symbol-packs' spells it: the file itself under
-- 'functions', the program it is fired against under 'input', every record the
-- protocol of '--evaluations' writes under 'protocol' and the program 𝕄 lands
-- on under 'result' — or the failure under 'fails'.
...
testSymbols pth = do
  ...
  loc <- parseExpressionThrows (fromMaybe "Q" location)
  withFunctionsOf (T.pack functions) $ \file -> do
    ...
    (_, written) <- recorded $ \record -> do
      ...
      case (result, fails) of
        ...
        (Nothing, Just message) ->
          morph expr emptyState ctx `shouldThrow` (\err -> message `isInfixOf` show (err :: SomeException))
        _ -> expectationFailure "The pack holds neither a single 'result' nor a single 'fails'"
    written `shouldBe` fromMaybe [] protocol
...
spec = do
  ...
  -- The whole of what a λ function answered by the '--functions' file does,
  -- pack by pack: the file itself, the program it is fired against, every
  -- record the protocol of '--evaluations' writes and the program 𝕄 lands on.
  describe "morph with the λ functions of '--functions'" $ do
  ...
  describe "morph with '_deep'" $ do
    ...
    describe "a dispatch naming an attribute of the formation it stands on" $
      it "cannot fire the λ the dispatch does not demand" $
        withFunctionsOf "- λ: L_answer\n  𝑛: ⟦ Δ ⤍ FF- ⟧\n" $ \file -> do
          box <- readLambdas file
          world <- parseExpressionThrows "[[ foo -> [[ f -> [[ a -> ?, @ -> $.a, L> L_answer ]] ]], x -> Q.foo.f( a -> [[ D> 01- ]] ).@ ]]"
          (morphed, _, _) <- morph world emptyState (withFunctions box (defaultReduceContext ExRoot)){_deep = True}
  ...
  describe "execBuildTerm 'evaluate'" $ do
    let univ = ExFormation []
        ctx = withFunctions known (defaultReduceContext ExRoot)
  ...
  describe "morphing 'md' is disjoint from 'ml'" $ do
    ...
    -- ⟦λ ⤍ F⟧.a and fires the λ. The chain therefore routes
    -- md → md → ml; firing the λ function no entry answers is what
    ...
    it "drills a chained λ-formation dispatch down to the base 'ml'" $ do
      ...
      morph' (chain, (ExRoot, Nothing) :| []) ExRoot emptyState (defaultReduceContext ExRoot)
        `shouldThrow` (\e -> "No entry of --functions answers the λ function 'F'" `isInfixOf` show (e :: SomeException))
