...
spec = do
    ...
    it "saves dataize steps to dir with --steps-dir" $
      withAtoms $ \atoms ->
        withTempDirectory "phino-steps-dataize" $ \dir ->
          withStdin "[[ bytes ↦ ⟦ φ ↦ ∅ ⟧, number(φ) -> [[ as-bytes -> $.φ, plus(x) -> [[ L> L_number_plus ]] ]], @ -> 5.plus(6).plus(7) ]]" $ do
            testCLISucceeded
  ...
  describe "dataize" $ do
    ...
    it "focuses a compressed sequence whose meet replaces a step root" $
      withAtoms $ \atoms ->
        withStdin "[[ @ -> [[ @ -> $.c.plus( 32.0 ), c -> 25.0 ]], bytes ↦ ⟦ φ ↦ ∅ ⟧, number(φ) -> [[ as-bytes -> $.φ, plus -> [[ x -> ?, L> L_number_plus ]] ]] ]]" $
          testCLISucceeded
    ...
    describe "--evaluations" $ do
      it "writes one tab-separated record per fired atom" $
        withAtoms $ \atoms ->
          withTempFile "evaluationsXXXXXX.txt" $ \(path, stream) -> do
            hClose stream
            withStdin "[[ bytes ↦ ⟦ φ ↦ ∅ ⟧, number(φ) -> [[ as-bytes -> $.φ, plus(x) -> [[ L> L_number_plus ]] ]], @ -> 5.plus(6) ]]" $
      ...
      it "writes a record for every firing" $
        withAtoms $ \atoms ->
          withTempFile "evaluationsXXXXXX.txt" $ \(path, stream) -> do
            hClose stream
            withStdin "[[ bytes ↦ ⟦ φ ↦ ∅ ⟧, number(φ) -> [[ as-bytes -> $.φ, plus(x) -> [[ L> L_number_plus ]] ]], @ -> 5.plus(6).plus(7) ]]" $
              testCLISucceeded ["dataize", atoms, "--evaluations=" ++ path, "--quiet", "--sweet", "--hide-rho"] []
            records <- readUtf8 path
      ...
      it "keeps the records of a run that fails" $
        withAtoms $ \atoms ->
          withTempFile "evaluationsXXXXXX.txt" $ \(path, stream) -> do
            hClose stream
            withStdin "[[ bytes ↦ ⟦ φ ↦ ∅ ⟧, number(φ) -> [[ as-bytes -> $.φ, plus(x) -> [[ L> L_number_plus ]], nope -> [[ L> L_number_nope ]] ]], @ -> 5.plus(6).nope ]]" $
              testCLIFailed
    ...
    describe "--partial" $ do
      let stuck = "[[ bytes ↦ ⟦ φ ↦ ∅ ⟧, number(φ) -> [[ as-bytes -> $.φ, times(x) -> [[ L> L_number_times ]], nope -> [[ L> L_number_nope ]] ]], @ -> 2.times(3).nope ]]"
    ...
    describe "--atoms" $ do
      let sum' = "[[ bytes ↦ ⟦ φ ↦ ∅ ⟧, number(φ) -> [[ as-bytes -> $.φ, plus(x) -> [[ L> L_number_plus ]] ]], @ -> 5.plus(6) ]]"
    ...
    describe "--inside" $ do
      let universe = "[[ bytes ↦ ⟦ φ ↦ ∅ ⟧, number(φ) -> [[ as-bytes -> $.φ, plus(x) -> [[ L> L_number_plus ]] ]], @ -> [[ D> 01- ]] ]]"
  ...
  describe "morph" $ do
    ...
    let chained = "[[ bytes ↦ ⟦ φ ↦ ∅ ⟧, number(φ) -> [[ as-bytes -> $.φ, plus(x) -> [[ L> L_number_plus ]] ]], @ -> 5.plus(6).plus(7) ]]"
    it "prints help" $
    ...
    describe "--deep" $ do
      let program =
            "[[ bytes ↦ ⟦ φ ↦ ∅ ⟧, \
            \number(φ) -> [[ as-bytes -> $.φ, times(x) -> [[ L> L_number_times ]] ]], \
            \bar(x) -> [[ L> L_bar ]], \
        ...
        withAtoms $ \atoms ->
          withStdin program $
            testCLISucceeded
              ...
              [ "number(φ) ↦ ⟦ as-bytes ↦ φ, times(x) ↦ ⟦ λ ⤍ L_number_times ⟧ ⟧"
