...
spec = do
  ...
  describe "morph" $ do
    ...
    describe "--deep" $ do
      ...
      -- An operand of a firing that reaches the terminator ⊥, here through the
      -- void 'data' nothing filled, parks that firing the way an operand parked
      -- on an unanswered λ function does, and the walk goes on (#1401)
      describe "an operand that reaches ⊥" $ do
        let world =
              unlines
                [ "⟦"
                , "  number ↦ ⟦ φ ↦ ∅ ⟧,"
                , "  bytes ↦ ⟦ φ ↦ ∅, size ↦ ⟦ λ ⤍ L_bytes_size ⟧ ⟧,"
                , "  dataized ↦ ⟦ target ↦ ∅, λ ⤍ L_dataized ⟧,"
                , "  block ↦ ⟦ data ↦ ∅, buffer ↦ ∅, read ↦ ⟦ φ ↦ Φ.dataized( target ↦ ξ.ρ.data.size ).as-bytes ⟧ ⟧,"
                , "  l🌵 ↦ ⟦"
                , "    mark ↦ ⟦ n ↦ ∅, v ↦ ∅, λ ⤍ L_entry ⟧,"
                , "    root ↦ ⟦ v ↦ ∅, λ ⤍ L_root ⟧,"
                , "    e1 ↦ Φ.l🌵.mark( n ↦ Φ.number( φ ↦ Φ.bytes( φ ↦ ⟦ Δ ⤍ 3F-F0-00-00-00-00-00-00 ⟧ ) ) )( v ↦ Φ.block( buffer ↦ Φ.bytes( φ ↦ ⟦ λ ⤍ 𝜎1 ⟧ ) ).read )"
                , "  ⟧"
                , "⟧"
                ]
            atoms =
            ...
            morphing :: FilePath -> FilePath -> [String]
            morphing lambdas protocol =
              ["morph", "--deep", "--acyclic", "--partial", "--sweet", "--hide-rho", "--flat", "--symbolic=" ++ lambdas, "--locator=Q.l🌵", "--protocol=" ++ protocol, "--max-steps=32"]
        it "parks the firing and exits successfully with --partial" $
          withTempFileContent "atomsXXXXXX.yaml" atoms $ \lambdas ->
            withTempFile "protocolXXXXXX.xml" $ \(path, stream) -> do
              hClose stream
        ...
        it "still fails on it without --partial" $
          withTempFileContent "atomsXXXXXX.yaml" atoms $ \lambdas ->
            withTempFile "protocolXXXXXX.xml" $ \(path, stream) -> do
              hClose stream
              withStdin world (testCLIFailed (filter (/= "--partial") (morphing lambdas path)) ["terminator ⊥"])
    ...
    it "explains normalization rules" $
      testCLISucceeded
        ...
        [ unlines
            ...
            , "  { [[ B_1, \\tau -> n, B_2 ]] = e_1 \\;\\text{and}\\; \\lparen [ L ] \\cap \\lparen B_1 \\cup B_2 \\rparen = \\emptyset \\;\\text{or}\\; [ D ] \\cap \\lparen B_1 \\cup B_2 \\rparen = \\emptyset \\rparen }"
            , "  { \\phinoContextualize{ n }{ [[ B_1, B_2 ]] }{ e_2 } }"
