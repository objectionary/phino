    ...
    it "explains morphing rules" $
      testCLISucceeded
        ...
        [ unlines
            ...
            , "  { }"
            , "  { where $ e \\coloneqq \\phinoEvaluate{ [[ B_1, L> F, B_2 ]] } $ }"
            , "\\phinoMorphingRule{dispatch}"
            , "  { \\mathbb{M}( n . \\tau ) }"
            , "  { \\mathbb{M}( \\mathcal{N}( e . \\tau ) ) }"
            , "  { }"
            , "  { where $ e \\coloneqq \\phinoMorph{ n } $ }"
            , "\\phinoMorphingRule{application}"
            , "  { \\mathbb{M}( n ( \\tau -> e ) ) }"
            , "  { \\mathbb{M}( \\mathcal{N}( e_1 ( \\tau -> e ) ) ) }"
            , "  { }"
            , "  { where $ e_1 \\coloneqq \\phinoMorph{ n } $ }"
            , "\\phinoMorphingRule{applicationa}"
            , "  { \\mathbb{M}( n ( \\alpha_{i} -> e ) ) }"
            , "  { \\mathbb{M}( \\mathcal{N}( e_1 ( \\alpha_{i} -> e ) ) ) }"
            , "  { }"
            , "  { where $ e_1 \\coloneqq \\phinoMorph{ n } $ }"
            ...
            , "  { if $ e \\not= Q $ }"
            , "  { where $ e \\coloneqq \\phinoGlobal{  } $ }"
            , "\\phinoMorphingRule{stuck}"
            , "  { \\mathbb{M}( n ) }"
            , "  { T }"
            , "  { }"
            , "  { }"
