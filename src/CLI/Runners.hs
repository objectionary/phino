  ...
  (outcome, chain, _) <-
    withEvalFunc
      ...
      ( \record -> do
          ...
          let ctx = ReduceContext loc _maxDepth _maxCycles (Steps _maxSteps 0) 1 _depthSensitive _shuffle _partial False False Dataization [] Map.empty lambdas buildTerm reduction evaluation fired save record
...
runMorph OptsMorph{..} = do
  ...
  (morphed, chain, _) <-
    withEvalFunc
      ...
      printCtx
      ( \record -> do
          let ctx = ReduceContext loc _maxDepth _maxCycles (Steps _maxSteps 0) 1 _depthSensitive _shuffle _partial _deep _acyclic Morphing [] Map.empty lambdas buildTerm reduction evaluation fired save record
          (universe, aiming) <- aimed _inside expr ctx
