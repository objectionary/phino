...
runMatch OptsMatch{..} = do
  ...
  if isNothing _pattern
    then logDebug "The --pattern is not provided, no substitutions are built"
      ...
      matches <- matchProgramWithRule prog (rule ptn condition) (RuleContext buildTerm)
      if null matches
        then throwIO EmptySubstsOnMatch
        else putStrLn (P.printSubsts' (map snd matches) (_sugarType, UNICODE, _flat, defaultMargin))
  where
